#!/usr/bin/env python
"""
Script to create a dataset using right-most subsequences from each series.
This approach:
1. Processes each series individually
2. Takes the N right-most subsequences from each series (most recent data)
3. Saves these subsequences to separate files
4. Optionally combines them into a single dataset
"""
...
def process_single_series_rightmost(
    config: DataConfig,
    series_id: str,
    ...
    """
    Process a single series, taking the right-most (most recent) subsequences.
    ...
    Args:
        ...
        subsequences_per_series: Maximum number of subsequences to take from this series
    ...
    try:
        ...
        # Create right-most subsequences
        all_X = []
        all_y = []
        
        # Start from the end of the series and work backwards
        # The last point is the target for the first subsequence
        total_points = len(scaled_values)
        
        # For each possible subsequence length (from full length down to 1)
        for i in range(1, min(sequence_length + 1, total_points)):
            ...
            # We take the last i values for the input sequence
            if i < total_points:
                padded_subsequence[-i:] = scaled_values[-(i+1):-1]
                target = scaled_values[-1]  # Last value is always the target
                
                # Add the sequence and target
                all_X.append(padded_subsequence)
                all_y.append(target)
        
        # If we need more subsequences, move the window back
        remaining_subsequences = subsequences_per_series - len(all_X)
        if remaining_subsequences > 0 and total_points > sequence_length + 1:
            # Start from the second-to-last point as target and move backwards
            for target_idx in range(total_points - 2, sequence_length - 1, -1):
                # Create full-length subsequence (no padding needed)
                subsequence = scaled_values[(target_idx - sequence_length):target_idx]
                target = scaled_values[target_idx]
                
                all_X.append(subsequence)
                all_y.append(target)
                
                remaining_subsequences -= 1
                if remaining_subsequences <= 0:
                    break
        ...
        if len(all_X) < subsequences_per_series:
            if verbose:
                logger.warning(f"Series {series_id} has only {len(all_X)} subsequences, " 
                              f"which is less than the requested {subsequences_per_series}")
            
            # If we don't have enough, use all available with replacement
            if len(all_X) > 0:  # Only if we have at least one subsequence
                ...
                # If we have no subsequences, return failure
                return False
        else:
            # Take the first subsequences_per_series subsequences
            # These are already the right-most (most recent) ones
            X = np.array(all_X[:subsequences_per_series], dtype=dtype)
            y = np.array(all_y[:subsequences_per_series], dtype=dtype)
...
def create_rightmost_dataset(
    ...
    """
    Create a dataset using right-most subsequences from each series.
    
    Args:
        config: Data processing configuration
        start_series: Starting index for series (e.g., 1 for M1)
        end_series: Ending index for series (e.g., 48000 for M48000)
        subsequences_per_series: Number of subsequences to take from each series
        random_seed: Random seed for reproducibility
    ...
    try:
        ...
        if temp_dir is None:
            temp_dir = Path(config.processed_data_path) / "rightmost_samples"
        else:
        ...
        for series_id in tqdm(series_ids, desc="Processing series"):
            success = process_single_series_rightmost(
                config=config,
                series_id=series_id,
                train_df=train_df,
                output_dir=temp_dir,
        ...
        if combine:
            ...
            if start_series == 1 and end_series == 48000:
                output_suffix = f"rightmost_sampled{successful_series * subsequences_per_series}_seed{random_seed}"
            else:
                output_suffix = f"M{start_series}_M{end_series}_rightmost_sampled{successful_series * subsequences_per_series}_seed{random_seed}"
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description='Create dataset with right-most subsequences from all series')
    parser.add_argument(
        '--config',
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=375,
        help='Number of subsequences to take from each series'
    )
    parser.add_argument(
        '--random-seed',
        type=int,
